...
for i in range(n):
    name = input(str(i+1) + 'th enterprise: ')
    profit_q1 = int(input('Profit for the 1st quarter: '))
    profit_q2 = int(input('Profit for the 2nd quarter: '))
    profit_q3 = int(input('Profit for the 3rd quarter: '))
    profit_q4 = int(input('Profit for the 4th quarter: '))
...
for name, profit in base_enterprise.items():
    print(f'Enterprise: {name} profit for the year - {sum(profit)}')
    total_profit += profit

avg_profit_total = sum(total_profit) / len(base_enterprise)
print(f'Average annual profit for all enterprises {avg_profit_total}')

print('Companies with above average profits:')
